...
from service.jwt_service import access_token_expires, refresh_token_expires
...
def user_signin(user:UserSignin, connection, response: Response):
...
def user_logout(access_token: str, refresh_token: str, connection, response: Response):
    jwt_service.add_to_blacklist(access_token, refresh_token, connection)
    response.delete_cookie(key="access_token")
    response.delete_cookie(key="refresh_token")
    
def user_read(access_token: str, refresh_token: str, connection, response: Response) -> User:
    claim = jwt_service.verify_jwt(access_token, refresh_token, connection, response)
    return find_user_by_user_id(claim.user_id, connection)
    
def user_read_all(access_token: str, refresh_token: str, connection, response: Response) -> list[User]:
    claim = jwt_service.verify_jwt(access_token, refresh_token, connection, response)
...
def user_update(access_token: str, refresh_token: str, user_update:UserSignup, connection, response: Response):
    claim = jwt_service.verify_jwt(access_token, refresh_token, connection, response)
    db_user = find_user_by_user_id(claim.user_id, connection)
    ...
    user_logout(access_token, refresh_token, connection, response)
    
def user_delete(access_token: str, refresh_token: str, connection, response: Response):
    claim = jwt_service.verify_jwt(access_token, refresh_token, connection, response)
    db_user = find_user_by_user_id(claim.user_id, connection)
...
def user_role_admin(access_token:str, refresh_token: str, connection, response: Response):
    claim = jwt_service.verify_jwt(access_token, refresh_token, connection, response)
    db_user = find_user_by_user_id(claim.user_id, connection)
...
def user_role_user(access_token:str, refresh_token: str, connection, response: Response):
    claim = jwt_service.verify_jwt(access_token, refresh_token, connection, response)
    db_user = find_user_by_user_id(claim.user_id, connection)
